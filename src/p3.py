...
def create_network():
    ...
    for u, v in G.edges:
        if (u, v) in ring_edges or (v, u) in ring_edges:
            G.edges[u, v]['capacity'] = 200_000_000
        else:
            G.edges[u, v]['capacity'] = 300_000_000
...
def experiment_increasing_traffic():
    G, N_base = create_network()
    p = 0.9
    T_max = 10
    m = 1000
    ...
    print("Natężenie ruchu | Niezawodność Pr[T < T_max]")
    print("--------------------------------------------")
    for mult in multipliers:
        N_scaled = (N_base * mult).astype(int)
        calculate_actual_flow(G, N_scaled)
        reliability = simulate_network_reliability(G, N_scaled, p, T_max, m, num_simulations)
        print(f"{mult:>12}x | {reliability:>10.4f}")
        results.append((mult, reliability))
    return results

def main():
    experiment_increasing_traffic()
